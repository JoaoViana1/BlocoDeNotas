...
def new_file() -> str:
    '''Reset body and infobar, and clear filename variable'''
...
def open_file() -> str:
    ...
    try:
        filename: str = sg.popup_get_file('Abrir', no_window=True)
    except:
        return
...
def save_file_as() -> str:
    '''Save file instantlu if already open; otherwise use `save-as` popup'''
    if filename not in (None, ''):
        with open(filename, 'w') as f:
            f.write(values.get('_BODY_'))
        window['_INFO_'].update(value=filename)
...
def about_me():
    sg.PopupQuick('----------nome----------', auto_close=False)
while True:
